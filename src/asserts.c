...
bool assert_load_equals(FILE *fp, char *expected) {
  ...
  if (!assert_equals_unsigned_int("board height", expectedLines, game->num_rows)) {
    return false;
  }
...
bool assert_map_equals(game_t *game, unsigned int row, unsigned int col, char expected) {
  char actual = get_board_at(game, row, col);
  if (expected != actual) {
...
// Note: This function only works for games created from create_default_game.
bool assert_game_equals(game_t *expected, game_t *actual) {
  ...
  // Note: This code checks all snakes, so there may be errors if game->snakes was not allocated correctly.
